...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run OriginPlot v5.8.9-p13 full test suite.")
    ...
    payload = {"schema": "originplot.run_all_tests.v5.8.9-p13", "skill_version": "5.8.9-p13", "status": "ok" if not compile_failures and not test_errors and skipped == 0 and not discovery_failures else "failed", "compile_failures": compile_failures, "tests_run": tests_run, "expected_min_tests": args.expected_min_tests, "skipped": skipped, "test_errors": test_errors, "discovery_failures": discovery_failures}
