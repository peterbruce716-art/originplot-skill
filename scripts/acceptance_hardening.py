...
FIG12_DEFAULTS = {
    ...
    "fig12_label_sizes": {
        "panel": 10.0, "contour": 8.4, "mechanism": 11.2,
...
def evaluate_target_visual_gate(
    ...
    *,
    ...
    provisional = figure == "fig16"
    promoted = not failures and not provisional
    status = "provisional" if provisional and not failures else "promoted" if promoted else "not_promoted"
    return {
        "schema": "originplot.target_visual_gate.v1",
        "figure_id": figure,
        "baseline_role": "frozen_regression" if figure == "fig15" else "provisional" if figure == "fig16" else "candidate_baseline",
        "thresholds": TARGET_VISUAL_GATES[figure],
        "values": {name: values.get(name) for name in TARGET_VISUAL_GATES[figure]},
        "checks": checks,
        "failures": failures,
        "frozen_identity_recognized": bool(frozen_identity_recognized) if figure == "fig15" else None,
...
def normalize_fig12_parameters(requested: dict[str, Any], *, strict: bool) -> dict[str, Any]:
    ...
    for name in ("fig12_panel_layout_offsets", "fig12_matrix_biases", "fig12_matrix_contrasts"):
