...
EXPECTED_SKILL_VERSION = "5.8.9-p13"
...
def final_release_bundle_gate(skill_root: Path, release_bundle: Path | None) -> dict[str, Any]:
    ...
    validator = load_module(
        "originplot_p13_release_bundle_validator", skill_root / "scripts" / "validate_release_bundle.py"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate OriginPlot v5.8.9-p13 release readiness.")
