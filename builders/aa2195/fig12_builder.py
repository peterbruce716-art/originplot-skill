...
from .geometry import fig12_panels
...
FIG12_DEFAULT_MATRIX_BIASES = {0: -1.5, 1: 0.0, 2: 0.0}
...
FIG12_DEFAULT_MATRIX_SMOOTHING_SIGMA = 1.25
...
def build(op: Any, candidate_params: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, (panel, layer) in enumerate(zip(panels, layers)):
        matrix_values = _apply_matrix_bias(
            panel["matrix"], panel["levels"], matrix_biases[index], matrix_contrasts[index]
        )
