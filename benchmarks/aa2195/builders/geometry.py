...
def _fig12_source_matrix(
    ...
    region_values = _fig12_threshold_centered_region_values(levels)
...
def _fig12_threshold_centered_region_values(levels: list[float]) -> np.ndarray:
    """Choose three region values whose interpolated boundaries hit L1/L2 at 50%."""
    if len(levels) != 4:
        raise ValueError("Fig12 source digitization requires four contour levels")
    low, lower_boundary, upper_boundary, high = map(float, levels)
    if not low < lower_boundary < upper_boundary < high:
        raise ValueError("Fig12 contour levels must be strictly increasing")

    # The middle value must also keep the outer region values inside the
    # declared contour range. Taking the center of the feasible interval
    # maximizes the margin to both outer limits.
    middle_low = max(lower_boundary, 2.0 * upper_boundary - high)
    middle_high = min(upper_boundary, 2.0 * lower_boundary - low)
    if middle_low > middle_high:
        raise ValueError("Fig12 contour levels cannot support centered region thresholds")
    middle = (middle_low + middle_high) / 2.0
    return np.asarray(
        [2.0 * upper_boundary - middle, middle, 2.0 * lower_boundary - middle],
        dtype=float,
    )
