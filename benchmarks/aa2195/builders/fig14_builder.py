...
def _style_marker(plot: Any, color: str, symbol: int) -> None:
    """Style the native scatter overlay without introducing a connecting line."""
    _style_symbol(plot, color, symbol)


def _patterned_series(x: list[float], y: list[float], mode: str) -> tuple[list[float], list[float]]:
    """Keep the freshly digitized anchors; Origin supplies the native dash pattern."""
    return list(x), list(y)


def _error_paths(temperature: list[float], y: list[float], err: list[float]) -> tuple[list[float], list[float]]:
    xs: list[float] = []
    ys: list[float] = []
    half_cap = 3.0
...
def build(op: Any, candidate_params: dict[str, Any]) -> dict[str, Any]:
    fresh_source = load_fresh_figure_data(candidate_params, "fig14")
    temperature = [float(value) for value in fresh_source["data"]["temperature"]]
    series = fresh_source["data"]["series"]
    ...
    for mode in ("PSC", "UC", "TR"):
        record = series[mode]
        error_x, error_y = _error_paths(temperature, record["y"], record["err"])
        ...
        markers = layer.add_plot(sheet, colx=marker_column, coly=marker_column + 1, type="scatter")
        _style_marker(markers, record["color"], int(record["symbol"]))
        marker_plot_numbers[mode] = plot_count + 1
        contracts.append({"layer_index": 0, "plot_index": plot_count, "plot_type_code": 201, "x_column": chr(ord("A") + marker_column), "y_column": chr(ord("A") + marker_column + 1)})
        plot_style_contracts.append({
            "layer_index": 0,
            "plot_index": plot_count,
            "plot_type_code": 201,
        ...
        source_groups.append({
            ...
            "canonical_source": {
                "source_id": f"fresh_source_bundle.fig14.{mode}",
                "kind": "fresh_crop_temperature_fraction_error_anchors",
            },
    ...
    return {
        ...
        "reproduction_mode": {
            "fresh_extract": "fresh_source_reconstructed_approximate",
            "validated_reuse": "validated_reuse_reconstructed_approximate",
            "validated_crop_reextract": "validated_crop_reextract_reconstructed_approximate",
        }[fresh_source["source_data_policy"]],
        "source_data_policy": fresh_source["source_data_policy"],
        "data_provenance": "hash-verified AA2195 Fig. 14 source-bundle markers and error bars",
