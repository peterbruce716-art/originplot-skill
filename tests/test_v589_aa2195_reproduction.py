...
class FakeBuilderOrigin:
    def __init__(self) -> None:
        ...
        self.application_show_history: list[bool] = []

    def set_show(self, value: bool) -> None:
        self.application_show_history.append(bool(value))
...
class WorksheetReadbackGateTests(unittest.TestCase):
    ...
    def test_fig12_direct_worksheet_gate_requires_reopened_contour_levels(self) -> None:
        from builders.aa2195.readback import validate_direct_worksheet_plot_bindings
        ...
        plot = {
            ...
            "y_column": "B",
            "z_column": "C",
            "zlevels": {"minors": 0, "levels": [27.9, 35.55, 41.08, 54.0]},
        }
        contract = {
            "layer_index": 0,
            "plot_index": 0,
        ...
        passed = validate_direct_worksheet_plot_bindings(
            {"layers": [{"index": 0, "plot_details": [plot]}]},
            [contract],
        )
        plot["zlevels"] = {"minors": 0, "levels": [27.9, 36.6, 45.3, 54.0]}
        failed = validate_direct_worksheet_plot_bindings(
            {"layers": [{"index": 0, "plot_details": [plot]}]},
            [contract],
        )
        ...
        self.assertEqual("failed", failed["status"])
        self.assertTrue(
            any(item["property"] == "zlevels.levels" for item in failed["mismatches"])
        )
...
class AuthorizedSessionTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.process_inventory = patch(
            "builders.aa2195.session.origin_process_inventory",
            return_value=[self.formal_process],
        )
        self.process_inventory.start()

    def tearDown(self) -> None:
        self.process_inventory.stop()

    def test_authorized_session_rejects_attach_that_spawns_embedding_process(self) -> None:
        from builders.aa2195 import session
        ...
        after = [
            {
                "pid": 8432,
                "visible": False,
                "command_line": "C:/program/origin2022/Origin64.exe -Embedding",
            }
        ]
        op = FakeOrigin()
        with patch.object(session, "is_administrator_python", return_value=True), patch.object(
            session, "origin_process_inventory", side_effect=[before, after], create=True
        ):
            with self.assertRaisesRegex(RuntimeError, "E123_ORIGIN_SESSION_IDENTITY_DRIFT"):
                with session.origin_session(op, attach_existing_authorized=True):
                    pass

        self.assertEqual([("attach", None), ("detach", None)], op.calls)

    def test_authorized_session_records_stable_visible_origin_pid(self) -> None:
        from builders.aa2195 import session
        ...
        with patch.object(session, "is_administrator_python", return_value=True), patch.object(
            ...
            with session.origin_session(op, attach_existing_authorized=True) as evidence:
                self.assertEqual(16900, evidence["origin_pid"])
                self.assertTrue(evidence["origin_window_visible"])
                self.assertFalse(evidence["origin_embedding"])
...
class GeometryContractTests(unittest.TestCase):
    def test_fig12_matrix_ranker_reconstructs_source_row_orientation(self) -> None:
        from scripts.rank_fig12_matrix_candidates import reconstruct_source_grid

        matrix = np.asarray([[10.0, 20.0], [30.0, 40.0]])
        reconstructed = reconstruct_source_grid(matrix, width=2, height=2)
        np.testing.assert_allclose(
            np.asarray([[30.0, 40.0], [10.0, 20.0]]),
            reconstructed,
        )

    def test_fig12_matrix_ranker_maps_contour_intervals_to_palette_classes(self) -> None:
        from scripts.rank_fig12_matrix_candidates import classify_contour_regions

        levels = [27.9, 35.55, 41.08, 54.0]
        values = np.asarray([[30.0, 38.0, 47.0]])
        np.testing.assert_array_equal(
            np.asarray([[2, 1, 0]]),
            classify_contour_regions(values, levels),
        )

    def test_fig12_matrix_ranker_tolerant_boundary_accepts_small_grid_shift(self) -> None:
        from scripts.rank_fig12_matrix_candidates import _boundary_f1, _tolerant_boundary_f1

        expected = np.zeros((12, 12), dtype=int)
        actual = np.zeros((12, 12), dtype=int)
        expected[:, 5:] = 1
        actual[:, 7:] = 1
        valid = np.ones((12, 12), dtype=bool)

        self.assertEqual(0.0, _boundary_f1(expected, actual, valid))
        self.assertEqual(
            1.0,
    ...
    def test_fig12_origin_response_calibration_ranks_identity_for_matching_classes(self) -> None:
        from scripts.calibrate_fig12_origin_response import compare_panel_response

        predicted = np.zeros((12, 14), dtype=int)
        predicted[:, 4:9] = 1
        predicted[:, 9:] = 2
        valid = np.ones(predicted.shape, dtype=bool)

        result = compare_panel_response(predicted, predicted.copy(), valid)

        self.assertEqual("identity", result[0]["transform"])
        self.assertEqual(1.0, result[0]["class_accuracy"])
        self.assertEqual(0.0, result[0]["area_fraction_l1"])

    def test_fig12_origin_response_calibration_includes_equal_level_hypotheses(self) -> None:
        from scripts.calibrate_fig12_origin_response import contour_level_hypotheses

        result = contour_level_hypotheses(
            [45.7, 49.77, 60.17, 67.2],
            np.asarray([[48.0, 63.0]]),
        )

        self.assertEqual([45.7, 49.77, 60.17, 67.2], result["published_nonuniform"])
        np.testing.assert_allclose(
            [45.7, 52.8666666667, 60.0333333333, 67.2],
            result["equal_published_range"],
        )
        np.testing.assert_allclose([48.0, 53.0, 58.0, 63.0], result["equal_data_range"])
    ...
    def test_fig12_region_values_center_both_nonuniform_contour_thresholds(self) -> None:
        from builders.aa2195.geometry import _fig12_threshold_centered_region_values

        for levels in (
            [27.90, 35.55, 41.08, 54.00],
            [30.20, 37.79, 45.19, 56.90],
            [45.70, 49.77, 60.17, 67.20],
        ):
            high, middle, low = _fig12_threshold_centered_region_values(levels)
            self.assertAlmostEqual(levels[2], (high + middle) / 2.0)
            self.assertAlmostEqual(levels[1], (middle + low) / 2.0)
            self.assertGreaterEqual(low, levels[0])
            self.assertLessEqual(high, levels[3])

    def test_fig12_region_value_remap_preserves_contour_class_order(self) -> None:
        import numpy as np

        from builders.aa2195.fig12_builder import _apply_matrix_region_values

        levels = [45.70, 49.77, 60.17, 67.20]
        matrix = np.asarray([[46.05, 53.49, 66.85]], dtype=float)
        remapped = _apply_matrix_region_values(matrix, levels, [67.1, 59.2, 49.05])
        self.assertTrue(np.allclose(remapped, [[49.05, 59.2, 67.1]]))
        with self.assertRaisesRegex(ValueError, "preserve low/middle/high"):
            _apply_matrix_region_values(matrix, levels, [48.0, 59.2, 49.05])
...
class FigureBuilderContractTests(unittest.TestCase):
    ...
    def test_fig12_builder_uses_calibrated_matrices_and_layout(self) -> None:
        ...
        self.assertEqual({0: 0.0, 1: 0.0, 2: 0.0}, result["fig12_matrix_biases"])
        ...
        self.assertEqual("Times New Roman", result["fig12_axis_font"])
    ...
    def test_fig12_path_overlay_svg_is_source_derived_and_portable(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            image = Image.new("RGB", (805, 590), "white")
            draw = ImageDraw.Draw(image)
            draw.rectangle((99, 54, 180, 268), fill=(252, 191, 110))
            draw.rectangle((181, 54, 250, 268), fill=(177, 223, 137))
            draw.rectangle((251, 54, 320, 268), fill=(198, 223, 236))
            image.save(source)
            contours, paths = _fig12_boundary_svg("PSC", source, output)
            svg = output.read_text(encoding="utf-8")

        self.assertGreater(contours, 0)
        self.assertGreater(paths, 0)
        self.assertIn('fill="#fcbf6e"', svg)
        self.assertIn('fill="#b1df89"', svg)
        self.assertIn('fill="#c6dfec"', svg)
        self.assertIn('stroke="#708e57"', svg)
        self.assertIn('stroke-width="0.5"', svg)

    def test_fig12_path_overlay_flag_is_strict_boolean(self) -> None:
        from builders.aa2195.fig12_builder import _fig12_path_overlays_enabled

        self.assertTrue(_fig12_path_overlays_enabled({"fig12_path_overlays": True}))
        self.assertFalse(_fig12_path_overlays_enabled({"fig12_path_overlays": "true"}))
    ...
    def test_fig12_builder_applies_explicit_label_size_candidate_without_changing_default(self) -> None:
        ...
        self.assertEqual(11.0, result["fig12_label_sizes"]["panel"])
        self.assertAlmostEqual(9.6, result["fig12_label_sizes"]["contour"])
        self.assertAlmostEqual(11.8, result["fig12_label_sizes"]["mechanism"])
        ...
        self.assertTrue(all(record["size"] == 11.8 for record in mechanism_labels))
...
class VisualQaTests(unittest.TestCase):
    def test_candidate_worker_preserves_stable_runtime_error_code(self) -> None:
        from scripts import origin_candidate_worker

        error = RuntimeError(
            "E123_ORIGIN_SESSION_IDENTITY_DRIFT: attached process changed"
        )
        self.assertEqual(
            "E123_ORIGIN_SESSION_IDENTITY_DRIFT",
            origin_candidate_worker.stable_runtime_error_code(error),
        )

    def test_pre_save_demo_gate_rejects_cyan_watermark(self) -> None:
        from builders.aa2195 import session

        with tempfile.TemporaryDirectory() as tmp:
            image_path = Path(tmp) / "pre-save.png"
            ...
            gate = getattr(session, "assert_no_demo_watermark", None)
    ...
    def test_candidate_worker_records_effective_builder_route(self) -> None:
        ...
        self.assertEqual("frozen_regression", manifest["target_visual_gate"]["baseline_role"])
