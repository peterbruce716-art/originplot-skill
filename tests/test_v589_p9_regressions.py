...
class Fig16PersistentRectangleRegressionTests(unittest.TestCase):
    def test_graphobject_contract_rejects_wrong_object_type(self) -> None:
        from builders.aa2195.readback import validate_graphobject_contracts

        record = {
            "name": "fig12_boundary_a",
            "object_type": 8,
            "x": 210.0,
            "y": 429.0,
            "dx": 224.0,
        ...
        readback = {
            "layers": [{"graph_object_readback": {
                "objects": [record], "enumerated_objects": [record],
                "missing_names": [], "enumeration_status": "ok",
            }}]
        }
        result = validate_graphobject_contracts(
            readback,
            {"fig12_boundary_a": {"object_type": 34}},
        )
        self.assertEqual("failed", result["status"])
        self.assertIn("object_type", {item["property"] for item in result["mismatches"]})
