...
def _object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise OriginPlotError(
            "E210_AA2195_CONFIG_INVALID", f"{path.name} must contain an object"
        )
    return payload
...
def load_config() -> dict[str, Any]:
    ...
    if missing_routes:
        raise OriginPlotError(
            "E210_AA2195_CONFIG_INVALID", f"routes missing: {', '.join(missing_routes)}"
        )
    if sorted(routes["geometry_table_versions"]) != sorted(FIGURES):
        raise OriginPlotError(
            "E210_AA2195_CONFIG_INVALID",
            "geometry_table_versions must cover all AA2195 figures",
        )
    for key, digest in config["source_identities"].items():
        if not re.fullmatch(r"[0-9a-f]{64}", str(digest)):
            raise OriginPlotError(
                "E210_AA2195_CONFIG_INVALID",
                f"invalid SHA256 in source_identities: {key}",
            )
    return config
